...
# pois = read_csv('rs_I05' + '_' + 'MODIS-Aqua' + '_' + 'L2' + '_' + 'OC' + '.csv',
#       names=['id', 'dt', 'lat', 'lon', 'image_names'], parse_dates=[1])
#             #pois.image_names.replace('[]', np.nan, inplace=True)
# pois.dropna(axis=0, inplace= True)
# print(pois)
...
#                 # Download data
#                 handle = open(i, "wb")
#                 totlen = len(r.content) # size file
#                 tstart = time.time() # ref time download start
#                 dl = 0 # downloaded size
#                 for chunk in r.iter_content(chunk_size=512):
#                     print('ping')
#                     # if chunk:  # filter out keep-alive new chunks
#                     dl = dl + len(chunk)
#                     if (time.time() - tstart) > 0: # work around delta time of first iteration == 0
#                         speed = dl/(time.time() - tstart)
#                     else:
#                         speed = 2500
#                     if (time.time() - tstart) < 2 * totlen / speed and dl < totlen:
#                         handle.write(chunk)
#                     elif (time.time() - tstart) < 2 * totlen and dl == totlen:
#                         handle.write(chunk)
#                         handle.close()
#                         print('Done')
#                     else:
#                         print('Download timed out, connection may be closed by remote host, reconnecting ...')
#                         # handle.close()
#                         with requests.Session() as s:
#                             # Login to EarthData
#                             s.auth = (username, password)
...
#                                     else:
#                                         # handle.close()
#                                         print('Connection failed, download aborted.\n'
#                                             '\t- Check login/username.\n'
#                                             '\t- Check for connection problems: https://oceancolor.gsfc.nasa.gov/forum/oceancolor/topic_show.pl?tid=6447\n'
#                                             '\t- Check for blocked IP emailing: connection_problems@oceancolor.gsfc.nasa.gov\n')
#                                         # return None
...
#                 except requests.exceptions.ConnectionError:
#                     print 'Build https connection failed'
#                 except socket.timeout:
...
#             s.auth = (username, password)
#             r1 = s.request('get', url)
#             r = s.get(r1.url, auth=(username, password), stream=True, timeout=10)
#             if r.ok:
#                 if verbose:
#                     print('Downloading ' + i)
#                 # Download data
...
#                 # return None
#     except requests.exceptions.ConnectionError:
#         print('Build https connection failed, download failed')
#         handle.close()
#     except socket.timeout:
#         print('Download failed')
#         handle.close()
